...
from datasets.mini_ImageNet_support_query import MiniImageNet
import clip
from utils import *
...
def run_tip_adapter_F(cfg, cache_keys, cache_values, test_features, test_labels, clip_weights, clip_model, train_loader_F):
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            with torch.no_grad():
                image_features = clip_model.encode_image(images)#256*1024
                image_features /= image_features.norm(dim=-1, keepdim=True)

            affinity = adapter(image_features)#可以把两个矩阵相乘理解为矩阵通过一个线性变换后的结果
            cache_logits = ((-1) * (beta - beta * affinity)).exp() @ cache_values
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(datasets.query_set, batch_size=64, num_workers=8, shuffle=False)
    train_loader_cache = torch.utils.data.DataLoader(datasets.support_set, batch_size=256, num_workers=8, shuffle=False)
    train_loader_F = torch.utils.data.DataLoader(datasets.support_set, batch_size=256, num_workers=8, shuffle=True)

    # Textual features
    print("Getting textual features as CLIP's classifier.")
    clip_weights = clip_classifier(datasets.classnames1, datasets.template, clip_model)#获取文本特征1024*100
    print("Textual features shape ", clip_weights.shape)

    # Construct the cache model by few-shot training set
    print("\nConstructing cache model by few-shot visual features and labels.")
    cache_keys, cache_values = build_cache_model(cfg, clip_model, train_loader_cache)#构建缓存器模型
    ...
    test_features, test_labels = pre_load_features(cfg, "test", clip_model, test_loader)#载入测试数据被CLIP模型处理后的特征
